'''Week 08 Prove Data Structure -  Binary Tree'''
...
def print_tree(node):
    ...
    # base case to handle an empty 'None' node
    if node == 'None':
        return

    # base case
    elif node:
        # printed left to right because 8 left and 99 right
        # recursion
        print_tree(node.left)  # left nodes
        print(node.data)
        # recursion
        print_tree(node.right)  # right nodes
